...
class ShopifyOrderController extends Controller
{
    public function store(Request $request)
    {
        $request->validate([
            ...
            'orders.*.customer_name' => 'required',
            'orders.*.customer_phone' => 'required',
            'orders.*.customer_email' => 'required',
        ]);
        ...
        $existingOrderNumbers = $existingOrders->map(function($order) use($request){
            return $order->order_number && $order->user_id === $request->user_id;
        });

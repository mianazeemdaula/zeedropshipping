...
<div class="mx-auto">
    <div class="px-4 sm:px-8 md:px-12 bg-white rounded-lg mt-7 pt-2">
        ...
        @endif
        <form action="{{ route('vendor.bank-account.update', $account->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <x-input name="account_name" value="{{ $account->account_name }}" />
                </div>
                <div class="flex flex-col gap-2">
                    <x-label>Account Number (IBAN)</x-label>
                    <x-input name="iban" value="{{ $account->iban }}" />
                </div>
